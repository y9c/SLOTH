...
"""select UMI cluster from fq file."""
...
def filter_clustered_read(input_file):
    ...
    cluster proportion > x cluster read number > x read indel proportion
    < x
    """
    ...
    for umi_id, cluster_dict in cluster_counter.items():
        ...
        for cluster_id, cluster_size in cluster_dict.items():
            ...
            organ_umi_id = umi_id.replace("-", "-UMI")
            res.id = f"{organ_umi_id}({umi_size})-{cluster_id}({cluster_size})"
            res.description = res.description.split()[1]
            if res.seq.count("-") <= 3:
